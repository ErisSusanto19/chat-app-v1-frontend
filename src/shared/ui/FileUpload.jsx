  ...
  return (
    <div>
      ...
      <div
        ...
        className={clsx(`
          relative flex justify-center items-center border-2 border-dashed cursor-pointer transition-colors
          ${circle ? 'w-48 h-48 rounded-full' : 'w-full h-48 rounded-md'}
          ${isDragActive ? 'border-[#545F71] bg-[#E9ECEF]' : 'border-gray-300 hover:border-[#545F71]'}
          ${errorMessage ? 'border-red-500 bg-red-50' : ''}
          ${currentValue ? 'border-solid p-0' : 'px-6 py-4'}
        `)}
      >
          ...
          <div className="text-center w-full h-full">
            ...
            {preview ? (
              <img 
                src={preview} 
                alt="File preview" 
                className={`${circle? 'w-full h-full rounded-full object-cover' : 'max-h-36 w-auto rounded-md object-contain'}`} />
            ) : (
